...
namespace ServerValueModifier.HarmonyOverrides
{
    ...
    public sealed class PrestigeAsyncPatch : AbstractPatch
    {
        ...
        {
            return typeof(PrestigeController).GetMethod(nameof(PrestigeController.GetPrestige))!;
        }
            ...
            try//try-catch in case no config
            {
                ...
                if (cf.Player.EnablePlayer)
                {
                    if (cf.Player.EnableHealth)
                    {
                        ...
                        LocalRaidPatch.HealthEdit(health, cf.Player.Health, "Maximum");
                    }
        ...
        public static bool CheckPocketQuest(PmcData pmcdata)
        {
...
//    ModHelper modhelper,
//    SaveServer saveServer) : PrestigeController( templateTable, profileHelper, saveServer)
//    {
//        public const string DEFAULT_POCKETS = "627a4e6b255f7527fb05a0f6";
//        public const string UNHEARD_POCKETS = "65e080be269cbd5c5005e529";
//        public const string CUSTOM_POCKETS = "a8edfb0bce53d103d3f62b9b";
...
//                PmcData? scavdata = profileHelper.GetScavProfile(sessionId);
//                var allPockets = pmcdata.Inventory.Items.Where(item => item.SlotId == "Pockets");
//                var hasCompletedPocketUpgradeQuest = CheckPocketQuest(pmcdata);
//                var isUnheard = pmcdata.Info.GameVersion == "unheard_edition";
//                //Attempt to revive pockets to default value.
